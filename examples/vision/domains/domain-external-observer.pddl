(define (domain vision)
    (:requirements :fluents :adl :typing)
    (:types gem - item - object)
    (:predicates
        (has ?i - item)
        (offgrid ?i - item)
        (visible ?i - item)
    )
    (:functions
        (xpos) (ypos) - integer
        (xloc ?o - object) (yloc ?o - object) - integer
        (walls)- bit-matrix
    )
    (:action pickup
        :parameters (?i - item)
        :precondition (and (not (has ?i)) (= xpos (xloc ?i)) (= ypos (yloc ?i)))
        :effect (and (has ?i) (offgrid ?i)
                     (assign (xloc ?i) -10) (assign (yloc ?i) -10))
    )
    (:action up
        :precondition (and (> ypos 1)
                        (= (get-index walls (- ypos 1) xpos) false))
        :effect (and (decrease ypos 1)
                    (forall (?i - item) ; 1-row in front
                        (when (or
                                (and (= (xloc ?i) xpos)
                                        (= (yloc ?i) (- ypos 2)))
                                (and (= (xloc ?i) (- xpos 1))
                                        (= (yloc ?i) (- ypos 2))
                                        (not (and (= (get-index walls (- ypos 1) (- xpos 1)) true)
                                                (= (get-index walls (- ypos 2) xpos) true))))
                                (and (= (xloc ?i) (+ xpos 1))
                                        (= (yloc ?i) (- ypos 2))
                                        (not (and (= (get-index walls (- ypos 1) (+ xpos 1)) true)
                                                (= (get-index walls (- ypos 2) xpos) true)))))
                            (visible ?i)))
                        (forall (?i - item) ; 2-row in front
                                (when (or
                                        (and (= (xloc ?i) xpos)
                                                (= (yloc ?i) (- ypos 3))
                                                (and (= (get-index walls (- ypos 2) xpos) false)))
                                        (and (= (xloc ?i) (- xpos 1))
                                                (= (yloc ?i) (- ypos 3))
                                                (and (= (get-index walls (- ypos 2) (- xpos 1)) false)
                                                    (= (get-index walls (- ypos 2) xpos) false)))
                                        (and (= (xloc ?i) (- xpos 2))
                                                (= (yloc ?i) (- ypos 3))
                                                (= (get-index walls (- ypos 2) (- xpos 1)) false)
                                                (not (and (= (get-index walls (- ypos 1) (- xpos 1)) true)
                                                        (= (get-index walls (- ypos 2) xpos) true)))
                                                (not (and (= (get-index walls (- ypos 2) (- xpos 2)) true)
                                                        (= (get-index walls (- ypos 3) (- xpos 1)) true))))
                                        (and (= (xloc ?i) (+ xpos 1))
                                                (= (yloc ?i) (- ypos 3))
                                                (and (= (get-index walls (- ypos 2) xpos) false)
                                                    (= (get-index walls (- ypos 2) (+ xpos 1)) false)))
                                        (and (= (xloc ?i) (+ xpos 2))
                                                (= (yloc ?i) (- ypos 3))
                                                (= (get-index walls (- ypos 2) (+ xpos 1)) false)
                                                (not (and (= (get-index walls (- ypos 1) (+ xpos 1)) true)
                                                        (= (get-index walls (- ypos 2) xpos) true)))
                                                (not (and (= (get-index walls (- ypos 2) (+ xpos 2)) true)
                                                        (= (get-index walls (- ypos 3) (+ xpos 1)) true)))))
                            (visible ?i)))))

    (:action down
        :precondition (and (< ypos (height walls))
                        (= (get-index walls (+ ypos 1) xpos) false))
        :effect (and
                    (increase ypos 1)
                    (forall (?i - item) ; 1-row in front
                        (when (or
                                (and (= (xloc ?i) xpos)
                                        (= (yloc ?i) (+ ypos 2)))
                                (and (= (xloc ?i) (- xpos 1))
                                        (= (yloc ?i) (+ ypos 2))
                                        (not (and (= (get-index walls (+ ypos 1) (- xpos 1)) true)
                                                (= (get-index walls (+ ypos 2) xpos) true))))
                                (and (= (xloc ?i) (+ xpos 1))
                                        (= (yloc ?i) (+ ypos 2))
                                        (not (and (= (get-index walls (+ ypos 1) (+ xpos 1)) true)
                                                (= (get-index walls (+ ypos 2) xpos) true)))))
                            (visible ?i)))
                        (forall (?i - item) ; 2-row in front
                                (when (or
                                        (and (= (xloc ?i) xpos)
                                                (= (yloc ?i) (+ ypos 3))
                                                (= (get-index walls (+ ypos 2) xpos) false))
                                        (and (= (xloc ?i) (- xpos 1))
                                                (= (yloc ?i) (+ ypos 3))
                                                (and (= (get-index walls (+ ypos 2) (- xpos 1)) false)
                                                        (= (get-index walls (+ ypos 2) xpos) false)))
                                        (and (= (xloc ?i) (- xpos 2))
                                                (= (yloc ?i) (+ ypos 3))
                                                (= (get-index walls (+ ypos 2) (- xpos 1)) false)
                                                (not (and (= (get-index walls (+ ypos 1) (- xpos 1)) true)
                                                        (= (get-index walls (+ ypos 2) xpos) true)))
                                                (not (and (= (get-index walls (+ ypos 2) (- xpos 2)) true)
                                                        (= (get-index walls (+ ypos 3) (- xpos 1)) true))))
                                        (and (= (xloc ?i) (+ xpos 1))
                                                (= (yloc ?i) (+ ypos 3))
                                                (and (= (get-index walls (+ ypos 2) xpos) false))
                                                        (= (get-index walls (+ ypos 2) (+ xpos 1)) false))
                                        (and (= (xloc ?i) (+ xpos 2))
                                                (= (yloc ?i) (+ ypos 3))
                                                (= (get-index walls (+ ypos 2) (+ xpos 1)) false)
                                                (not (and (= (get-index walls (+ ypos 1) (+ xpos 1)) true)
                                                        (= (get-index walls (+ ypos 2) xpos) true)))
                                                (not (and (= (get-index walls (+ ypos 2) (+ xpos 2)) true)
                                                        (= (get-index walls (+ ypos 3) (+ xpos 1)) true)))))
                                (visible ?i)))))

    (:action left
        :precondition (and (> xpos 1)
                        (= (get-index walls ypos (- xpos 1)) false))
        :effect (and (decrease xpos 1)
                    (forall (?i - item) ; 1-row in front
                        (when (or
                                (and (= (xloc ?i) (- xpos 2))
                                        (= (yloc ?i) ypos))
                                (and (= (xloc ?i) (- xpos 2))
                                        (= (yloc ?i) (- ypos 1))
                                        (not (and (= (get-index walls (- ypos 1) (- xpos 1)) true)
                                                (= (get-index walls ypos (- xpos 2)) true))))
                                (and (= (xloc ?i) (- xpos 2))
                                        (= (yloc ?i) (+ ypos 1))
                                        (not (and (= (get-index walls (+ ypos 1) (- xpos 1)) true)
                                                (= (get-index walls ypos (- xpos 2)) true)))))
                            (visible ?i)))
                        (forall (?i - item) ; 2-row in front
                                (when (or
                                        (and (= (xloc ?i) (- xpos 3))
                                                (= (yloc ?i) ypos)
                                                (= (get-index walls ypos (- xpos 2)) false))
                                        (and (= (xloc ?i) (- xpos 3))
                                                (= (yloc ?i) (- ypos 1))
                                                (and (= (get-index walls (- ypos 1) (- xpos 2)) false)
                                                    (= (get-index walls ypos (- xpos 2)) false)))
                                        (and (= (xloc ?i) (- xpos 3))
                                                (= (yloc ?i) (- ypos 2))
                                                (= (get-index walls (- ypos 1) (- xpos 2)) false)
                                                (not (and (= (get-index walls (- ypos 1) (- xpos 1)) true)
                                                        (= (get-index walls ypos (- xpos 2)) true)))
                                                (not (and (= (get-index walls (- ypos 2) (- xpos 2)) true)
                                                        (= (get-index walls (- ypos 1) (- xpos 3)) true))))
                                        (and (= (xloc ?i) (- xpos 3))
                                                (= (yloc ?i) (+ ypos 1))
                                                (and (= (get-index walls ypos (- xpos 2)) false)
                                                    (= (get-index walls (+ ypos 1) (- xpos 2)) false)))
                                        (and (= (xloc ?i) (- xpos 3))
                                                (= (yloc ?i) (+ ypos 2))
                                                (= (get-index walls (+ ypos 1) (- xpos 2)) false)
                                                (not (and (= (get-index walls (+ ypos 1) (- xpos 1)) true)
                                                        (= (get-index walls ypos (- xpos 2)) true)))
                                                (not (and (= (get-index walls (+ ypos 2) (- xpos 2)) true)
                                                        (= (get-index walls (+ ypos 1) (- xpos 3)) true)))))
                            (visible ?i)))))

    (:action right
        :precondition (and (< xpos (width walls))
                           (= (get-index walls ypos (+ xpos 1)) false))
        :effect (and (increase xpos 1)
                    (forall (?i - item) ; 1-row in front
                        (when (or
                                (and (= (xloc ?i) (+ xpos 2))
                                        (= (yloc ?i) ypos))
                                (and (= (xloc ?i) (+ xpos 2))
                                        (= (yloc ?i) (- ypos 1))
                                        (not (and (= (get-index walls (- ypos 1) (+ xpos 1)) true)
                                                (= (get-index walls ypos (+ xpos 2)) true))))
                                (and (= (xloc ?i) (+ xpos 2))
                                        (= (yloc ?i) (+ ypos 1))
                                        (not (and (= (get-index walls (+ ypos 1) (+ xpos 1)) true)
                                                (= (get-index walls ypos (+ xpos 2)) true)))))
                            (visible ?i)))
                        (forall (?i - item) ; 2-row in front
                                (when (or
                                        (and (= (xloc ?i) (+ xpos 3))
                                                (= (yloc ?i) ypos)
                                                (= (get-index walls ypos (+ xpos 2)) false))
                                        (and (= (xloc ?i) (+ xpos 3))
                                                (= (yloc ?i) (- ypos 1))
                                                (and (= (get-index walls (- ypos 1) (+ xpos 2)) false)
                                                    (= (get-index walls ypos (+ xpos 2)) false)))
                                        (and (= (xloc ?i) (+ xpos 3))
                                                (= (yloc ?i) (- ypos 2))
                                                (= (get-index walls (- ypos 1) (+ xpos 2)) false)
                                                (not (and (= (get-index walls (- ypos 1) (+ xpos 1)) true)
                                                        (= (get-index walls ypos (+ xpos 2)) true)))
                                                (not (and (= (get-index walls (- ypos 2) (+ xpos 2)) true)
                                                        (= (get-index walls (- ypos 1) (+ xpos 3)) true))))
                                        (and (= (xloc ?i) (+ xpos 3))
                                                (= (yloc ?i) (+ ypos 1))
                                                (and (= (get-index walls ypos (+ xpos 2)) false)
                                                    (= (get-index walls (+ ypos 1) (+ xpos 2)) false)))
                                        (and (= (xloc ?i) (+ xpos 3))
                                                (= (yloc ?i) (+ ypos 2))
                                                (= (get-index walls (+ ypos 1) (+ xpos 2)) false)
                                                (not (and (= (get-index walls (+ ypos 1) (+ xpos 1)) true)
                                                        (= (get-index walls ypos (+ xpos 2)) true)))
                                                (not (and (= (get-index walls (+ ypos 2) (+ xpos 2)) true)
                                                        (= (get-index walls (+ ypos 1) (+ xpos 3)) true)))))
                            (visible ?i)))))
)